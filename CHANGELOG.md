...
- **Modernization Intelligence opened on an empty subview** — The tab strip led with `Modernization Dashboard`, one of the six labelled placeholders, so the view opened on "ships with a later feature" by default. The four subviews that return data now lead.
