...
### Added
- **Program view** - `GET /api/modernization/program/{identity}` returns one program with its callees and callers, its copybooks and the ones the parser could not find, its parse fidelity and facts confidence, and the programs reachable from it by CALL. The closure is what makes the view usable for choosing conversion scope: converting a program whose callees are not converted leaves calls pointing at nothing. An ambiguous basename asks which one rather than picking, and a deps-only program says what is missing from it.
- **AST Explorer** — A dashboard tab that reads what the parser actually produced for one program: its sections and paragraphs, the raw AST as stored, and the COBOL behind any node. Ported from the preview branch with the four endpoints it renders (`/api/graph/rekt/files`, `/structure`, `/ast`, and `/api/source/content`). The source endpoint now honours `COBOL_SOURCE_FOLDER` rather than assuming `source`, and its containment check compares against the directory plus a separator so a sibling whose name merely starts with the same text is not served.
...
### Fixed
- **The program list offered programs that no longer existed** - The REKT files endpoint took the highest run id *per program* rather than the latest scan, so the graph's accumulation across runs was never pruned: every program any scan had ever seen stayed in the list. On an estate that had been rescanned this offered 173 programs where 32 were present, and a caller picking one would be working from a program that is no longer there. The unscoped request now reports the latest run and the programs belonging to it.
- **Generated code was filed where it could not compile** - A converted file's directory came from the converter's NamespaceName while its package or namespace declaration came from the model, and nothing reconciled the two. On a real estate every file landed in com/example/generated while declaring something else, so no Java in the output built as a project, and the run still reported success. The declaration now decides the directory, since that is what a compiler reads.
- **C# namespaces were treated as Java packages** - The declaration reader understood only Java's package form, so a C# file fell back to the default namespace, and the sanitiser lowercased what it did find, turning Contoso.Billing into contoso.billing and putting the file somewhere its own namespace did not describe. Both C# forms are now recognised, including the braced one, and case is preserved.
- **Conversion parity could not score a program staged in a subdirectory** — The post-pass identifies a program by file name, while the parser writes its report under the program's source-relative path, so on any estate that stages programs below the source root none of the candidate report names existed. Every such program was reported as having no structural context and the summary still printed an average over the few that remained: on a 36-program estate, 2 were scored and the mean of those two was presented as the result. The report is now also located by searching for it, accepting a unique match only, so a report found by basename alone is never attributed to one of several programs sharing that name.
- **Prompt Studio could silently remove a placeholder a prompt depends on** — Saving, generating or enhancing a prompt wrote the new text straight to disk. A template that loses `{{StartLine}}` or `{{CodebaseProfile}}` still renders and still reads correctly, so nothing fails; the model is simply given less and the output is quietly worse. An AI-assisted rewrite of every prompt removed four placeholders from the chunked converter and replaced `{{CodebaseProfile}}` with one estate's rendered summary, which would then have been used for every other estate. A save that would drop a placeholder is now refused, and a batch skips that prompt and names it rather than abandoning the run.
- **`--reuse-re` selected a run that held nothing** — The flag loaded business logic from the most recent run, which is not necessarily a run that persisted any: one terminated before its reverse engineering was written leaves a newer id with nothing behind it. A conversion then ran against the whole estate with no business-logic context, reported as a single warning among the startup output, and the generated code was materially worse for a reason nothing downstream recorded. The most recent run that actually holds business logic is now selected.
