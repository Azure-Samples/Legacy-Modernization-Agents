...
### Added
- **Focused Conversion Selectors** (preview) — Convert a named set of programs, optionally widened along recorded `CALL` edges, instead of the whole estate. `./doctor.sh run|convert-only --program <name|path>` accepts source-relative paths, basenames and stems, repeated or comma-separated, with `--include-callees` / `--include-callers` for a transitive dependency closure; the same resolution is available standalone as `resolve-programs` and in the portal behind a **Focused Convert** modal listing every program with its line count, parse fidelity and call counts. An ambiguous basename or stem is refused, naming every candidate, rather than resolved by picking one — converting the wrong program of two that share a name produces well-formed output that no downstream check can detect. Closure is computed from recorded facts, so with no scan it is reported as unavailable rather than empty, and a `CALL` whose target is only known at runtime is recorded in `unresolvedCallTargets` rather than dropped. The resolved scope is copied into `source/.conversion-staging/` — programs keeping their source-relative folders so same-named programs cannot collide, copybooks flat so `COPY` still resolves — and every run records a selection manifest with the selectors, the resolved programs, the reason each was included and the unresolved targets, so a narrow run can be told apart from a whole-estate run that lost programs. Portal conversions stage and record per run, so two started at once cannot overwrite each other's scope. Transaction, wave and target-component selectors are labelled placeholders pending the evidence they would select on. See [docs/focused-conversion-selectors.md](docs/focused-conversion-selectors.md).
