...
- **`tools/check-bash-compat.sh`** — portability lint for the project's shell scripts, covering the platform differences these scripts have actually broken on: here-documents inside command substitutions with a trailing `||`/`&&` (breaks bash 5.x / Git Bash), bash 4+ only syntax (breaks macOS `/bin/bash` 3.2), and CRLF line endings. Neither `bash -n` nor shellcheck reports these. Usage: `tools/check-bash-compat.sh [file ...]`.

### Fixed

- **Container startup failed silently and looked like a hang** — `ensure_rekt_containers` invoked the legacy `docker-compose` (v1) binary, which modern Docker Desktop no longer installs, with stderr redirected to `/dev/null`. The call exited 127 unnoticed, so the script then polled for containers that were never started and timed out after 60s with a misleading "did not become healthy" message. `doctor.sh` now resolves `docker compose` (v2) with a `docker-compose` fallback, preflights that Docker is installed *and* the daemon is reachable, and no longer suppresses Compose output. The same v1 invocation was fixed in `.devcontainer/devcontainer.json`.
- **Framework updates never rebuilt the Cobol-REKT image** — the `cobol-rekt` service declares both `image:` and `build:`, and Compose builds only when the image is absent. Anyone who had run an earlier version kept a stale image indefinitely and silently never received Dockerfile fixes. The image is now built with an `mma.rekt.fingerprint` label (hash of `tools/cobol-rekt/Dockerfile` plus `patches/*`) that is compared on every run, triggering exactly one rebuild when build inputs change. Unchanged checkouts skip both the pull and the build.
- **Compose failed on a fresh clone** — the portal's `env_file` list referenced the gitignored `Config/ai-config.local.env`. Compose treats `env_file` entries as mandatory, so every whole-project command (`docker compose up -d`, `config`, `ps`) aborted before starting a container. The entry is now `required: false` (Compose v2.24+) and `doctor.sh` creates an empty stub, so older Compose versions behave identically.
- **Container name conflicts across checkouts** — Compose derives its project name from the directory while every service pins `container_name:`, so a second checkout, a renamed folder, or a fresh clone during an upgrade failed with `Conflict. The container name "/cobol-rekt" is already in use`. The project name is now pinned (`name:` in `docker-compose.yml`, plus `COMPOSE_PROJECT_NAME` from `doctor.sh`), and containers left behind by a differently-named project are removed and recreated automatically. Graph data lives in named volumes and is unaffected.
- **Containerized portal could not reach the REKT graph** — the `portal` service set only `ApplicationSettings__Neo4j__Uri`, leaving `REKT_NEO4J_URI` at its `bolt://localhost:7688` default, which inside the container resolves to the container itself. It now points at `bolt://cobol-rekt-neo4j:7687` with matching credentials. The host portal was already correct and is unchanged.
- **Portal container permanently claimed port 5028** — the `portal` service published 5028 (the port `doctor.sh` runs the host portal on) with `restart: unless-stopped` and no profile, so a bare `docker compose up -d` claimed it and reclaimed it after every Docker restart, making the host portal fail with `AddressInUseException`. It now sits behind the `container-portal` profile (`docker compose --profile container-portal up -d portal`). Port cleanup also stops a *container* publishing the port, since a published port is held by Docker's proxy and killing that process does not release it.
- **Lower-case figurative constants silently degraded every affected program** — smojol's `FigurativeConstantMap` maps only the upper-case spellings of the twelve figurative constants (`ZERO`, `ZEROS`, `ZEROES`, `SPACE`, `SPACES`, `HIGH-VALUE(S)`, `LOW-VALUE(S)`, `QUOTE`, `QUOTES`, `NULL`) and throws `UnsupportedOperationException: Unsupported figurative constant: zero` on any other casing. COBOL is case-insensitive here, so ordinary source such as `MOVE zero TO WS-NUM` failed all three high-fidelity parse attempts and fell through to the deps-only fallback, losing the AST, CFG and data structures for the whole program. The staged copy is now normalised to upper case for programs and copybooks alike. This runs at staging time, immediately before smojol reads the files, rather than only in the preprocessor: the preprocessor writes to `source/.preprocessed/` only when it changes a file, and staging falls back to the raw source when it did not, so a preprocessor-only fix was bypassed for any program it did not otherwise touch — which is why affected programs still reported `No files needed preprocessing` and parsed at reduced fidelity. Comment lines and quoted literals are skipped, and hyphen is treated as a word character so identifiers such as `WS-ZERO-COUNT` are untouched. Original sources are never modified.
- **`rekt-full` failed on Git Bash with `syntax error near unexpected token '||'`** — the figurative-constant normalisation used a here-document inside a command substitution followed by `|| echo 0`. bash 3.2 (macOS) accepts this; bash 5.x, which Git Bash ships, re-parses the substitution body and rejects it. The pass was therefore skipped entirely on Windows, so lower-case figurative constants still degraded programs to a deps-only parse there. The guard and the fallback now sit outside the substitution. Neither `bash -n` nor shellcheck detects this — the error only surfaces at runtime — so `tools/check-bash-compat.sh` was added to catch it and related portability breaks.
- **`LENGTH OF` on a qualified reference produced invalid COBOL** — the preprocessor replaces `LENGTH OF <identifier>` with `0`, because smojol's grammar treats it as a CICS dialect token. It did not consume the `IN`/`OF` qualifiers that belong to the operand, so `MOVE LENGTH OF A IN B IN C` became `MOVE 0 IN B IN C`, qualifying a literal. smojol parsed that as a `MOVE` with no resolvable source operand and threw `NoSuchElementException` in `MoveFlowNode.resolve`, dropping the whole program to a deps-only result. The qualifiers are now consumed with the operand.
- **`source/.preprocessed/` went stale and silently changed parse results** — the directory was created once and never rebuilt, while staging prefers `.preprocessed/<file>` over the real source. A file is written there only when preprocessing *changes* it, so after a framework upgrade or a source edit that left a file needing no transformation, the previous version's output stayed behind and was parsed instead of the current source. Deleted or renamed programs orphaned their copies indefinitely, and bundled system copybooks were skipped when already present, so updated definitions shipped with the framework never reached existing checkouts. The directory is now rebuilt from scratch on every run — everything in it is derived and regenerated by the preprocessor's own phases. Preprocessor failures are no longer discarded either: an incomplete rebuild is reported with its error output, and if the preprocessor is unavailable the stale directory is dropped so parsing falls back to `source/`.
- **REKT parsing could stall indefinitely with no output** — each smojol invocation sent stdout to `/dev/null` and stderr to a log file, so a parse printed nothing at all while it ran and had no upper bound. A slow parse, a hung JVM and a call blocked by endpoint-protection software were indistinguishable, and a single file could hold the pipeline forever. Every invocation now prints elapsed seconds and is capped per file (`REKT_PARSE_TIMEOUT_SECONDS`, default 300s), enforced by polling rather than the `timeout` binary, which is absent on macOS. When a file times out, its remaining fallback attempts are skipped — those exist for parse *errors*, not hangs, and would otherwise multiply the wait by four.
- **Stale bind mounts were repaired with `docker restart`** — bind mounts are established at container creation, so a restart cannot re-point a mount whose host directory inode was replaced. Affected containers are now recreated with `--force-recreate`, and host bind-mount directories are created *before* containers start rather than afterwards.

### Added

- **`./doctor.sh containers {status|up|rebuild|reset}`** — inspect container and image state (flagging a stale REKT image), warm images ahead of time, force a rebuild after a framework update, or reset containers and graph volumes. Startup now pulls and builds with visible progress instead of hiding a multi-hundred-megabyte pull and a Maven build inside the first analysis run, uses `docker compose up -d --wait` against the existing healthchecks, and prints elapsed time plus real container state while waiting. Documented in [`docs/container-lifecycle.md`](docs/container-lifecycle.md).
- **`REKT_VERBOSE=1 ./doctor.sh rekt-full`** — streams live smojol output instead of the elapsed-time indicator, which is what distinguishes a genuinely slow parse (steady output) from one blocked by endpoint security or a proxy (no output at all). Full stdout and stderr for every file, successful or failed, is retained at `output/rekt/<program>.parse.log`, truncated per run so it always reflects the current attempt. Documented in [`docs/troubleshoot.md`](docs/troubleshoot.md).
- `.devcontainer/devcontainer.json` — forwards the REKT Neo4j ports 7475 (browser) and 7688 (bolt), which were previously unreachable from a devcontainer or Codespace.
