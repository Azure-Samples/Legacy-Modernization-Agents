...
- **Lower-case figurative constants silently degraded every affected program** — smojol's `FigurativeConstantMap` maps only the upper-case spellings of the twelve figurative constants (`ZERO`, `ZEROS`, `ZEROES`, `SPACE`, `SPACES`, `HIGH-VALUE(S)`, `LOW-VALUE(S)`, `QUOTE`, `QUOTES`, `NULL`) and throws `UnsupportedOperationException: Unsupported figurative constant: zero` on any other casing. COBOL is case-insensitive here, so ordinary source such as `MOVE zero TO WS-NUM` failed all three high-fidelity parse attempts and fell through to the deps-only fallback, losing the AST, CFG and data structures for the whole program. The preprocessor now upper-cases these words in the staged copy for both programs and copybooks. Comment lines and quoted literals are skipped, and hyphen is treated as a word character so identifiers such as `WS-ZERO-COUNT` are untouched. Original sources are never modified.
