Apply when changing code, tests, fixtures, or testing automation. 

Tests must protect observable behavior and data integrity.

- Establish expected behavior from requirements, supported contracts, and callers before choosing assertions. 
...
- Do not add tests that search source files for removed classes, obsolete strings, implementation snippets, or comments. Test the supported behavior. 
...
- Test names must describe what is actually exercised. 
...
- Assert failure, partial completion, cancellation, and unknown state distinctly. 
