...
PROJECT_DIR="$SCRIPT_DIR"  # Save the project directory
...
run_test() {
    echo -e "${BOLD}${BLUE}COBOL Migration Tool - Test Suite${NC}"
    echo "=================================="
    ...
    if [ -d "$SCRIPT_DIR/java-output" ]; then
        ...
        echo -e "${BLUE}ℹ️  Java output directory will be created during migration${NC}"
    fi
    ...
    if [ -d "$SCRIPT_DIR/csharp-output" ]; then
        ...
        if [ "$csharp_files" -gt 0 ]; then
            ...
            echo -e "${BLUE}ℹ️  No previous C# output found (will be created during migration)${NC}"
        fi
    else
        echo -e "${BLUE}ℹ️  C# output directory will be created during migration${NC}"
    fi
    ...
    if [ "$cobol_files" -gt 0 ]; then
        ...
        echo "  - Generate $cobol_files+ output files per language"
...
run_validate() {
    ...
    # Check directories
    for dir in "cobol-source" "java-output" "csharp-output"; do
...
main() {
    ...
    mkdir -p "$SCRIPT_DIR/cobol-source" "$SCRIPT_DIR/java-output" "$SCRIPT_DIR/csharp-output" "$SCRIPT_DIR/Logs"
